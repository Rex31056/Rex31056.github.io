...
if($exists==1)
{
  ...
  teleTotes,teleTrips,teleBins,telePoints,teleStackedTimes,teleBinLevel,teleComments) VALUES ('$matchNumber',
  ...
  '$teleTotes','$teleTrips','$teleBins','$telePoints','$teleStackedTimes','$teleBinLevel','$teleComments')");
  echo $telePoints;
//enter insert query
  // Hi Shannen,
...
if(!$q || !$q1)
 ...
 // $result = mysqli_query($con, "SELECT SUM(telePoints) AS sum FROM $tableName");
  ...
  mysqli_query($con, "UPDATE pit SET avgStacked=$avgPoints WHERE teamNum=$teamNumber");
  ...
  $result4 = mysqli_query($con, "SELECT SUM(stacke) AS sum FROM $tableName");
  $row4 = mysqli_fetch_assoc($result4);
  $sum4 = $row4['sum'];
  $avgTrips = $sum4/$numRow;
...
*/
  $sum5 = 2*$teleTotes + ($teleBins*$telBinLevel*2);
